...
import { CheckCircle2, Circle, Plus, Clock, TrendingUp, Play, Pause, Calendar, Edit2, Timer } from 'lucide-react';
...
export function Dashboard() {
  ...
  const totalTimeToday = todayTasks.reduce((sum, task) => sum + (task.duration || 0), 0);
  const timeSpentToday = todayTasks.reduce((sum, task) => {
    let spent = task.timeSpent || 0;
    if (task.inProgress && task.timerStartedAt) {
      const elapsed = Math.floor((currentTime.getTime() - new Date(task.timerStartedAt).getTime()) / 60000);
      spent += elapsed;
    }
    return sum + spent;
  }, 0);
  ...
  const toggleTask = (taskId: string) => {
    const updatedTasks = tasks.map(task => {
      if (task.id === taskId) {
        ...
        let finalTask = { ...task };
        if (task.inProgress && task.timerStartedAt) {
          const elapsed = Math.floor((new Date().getTime() - new Date(task.timerStartedAt).getTime()) / 60000);
  ...
  const formatTimerDisplay = (minutes: number, seconds: number) => {
    const hours = Math.floor(minutes / 60);
    const mins = minutes % 60;
    return `${String(hours).padStart(2, '0')}:${String(mins).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
  };

  const addQuickTask = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const saveTask = (taskId: string, newTitle: string, newType: 'work' | 'personal') => {
    const updatedTasks = tasks.map(task => {
      if (task.id === taskId) {
        return {
          ...task,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200">
            ...
            <p className="text-gray-900">{formatTime(timeSpentToday)} {totalTimeToday > 0 && `/ ${formatTime(totalTimeToday)}`}</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200">
            ...
            <p className="text-gray-900">{totalToday - completedToday}</p>
          </div>
        </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-gray-600">
                  {formatTime(timeSpentToday)} / {formatTime(totalTimeToday)}
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                <div 
                  className="bg-gradient-to-r from-green-500 to-green-600 h-full rounded-full transition-all duration-500"
                  style={{ width: `${Math.min((timeSpentToday / totalTimeToday) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
          )}
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 mb-8">
          {!showAddTask ? (
            ...
            <form onSubmit={addQuickTask} className="space-y-4">
              ...
              <div>
                <label className="block text-gray-700 mb-2">Expected Duration (minutes)</label>
                <select
                  name="duration"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
            ...
            {todayTasks.length === 0 ? (
              ...
              todayTasks
                ...
                .map(task => {
                  ...
                  const progress = task.duration && elapsed > 0 
                    ? Math.min((elapsed / task.duration) * 100, 100) 
                    : 0;
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 transition-colors ${
                        task.inProgress ? 'bg-indigo-50 border-l-4 border-indigo-600' : 'hover:bg-gray-50'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className={`text-gray-900 ${task.completed ? 'line-through text-gray-400' : ''}`}>
                              {task.title}
                            </h4>
                            {!task.completed && (
                              <button
                            ...
                            <div className="mb-3 p-4 bg-white border-2 border-indigo-600 rounded-lg">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="text-indigo-600 tabular-nums">
                                  {formatTimerDisplay(elapsed, seconds)}
                                </div>
                              </div>
                              {task.duration && (
                                <div className="mt-2">
                                  <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className="text-gray-900">
                                      {Math.round(progress)}% ({formatTime(elapsed)} / {formatTime(task.duration)})
                                    </span>
                                  </div>
                                  <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                    <div 
                            ...
                            {editingTaskId === task.id ? (
                              <div className="flex items-center gap-2">
                                <select
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (value) {
                                      updateTaskDuration(task.id, parseInt(value));
                                    }
                                  }}
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-1 rounded ${
                                  ...
                                  Expected: {task.duration ? formatTime(task.duration) : 'Not set'}
                                </span>
                                {!task.completed && (
                                  <button
                          ...
                          {task.duration && !task.completed && !task.inProgress && elapsed > 0 && (
                            <div className="mt-2">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-gray-600">Progress</span>
                                <span className="text-gray-600">
